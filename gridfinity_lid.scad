...
/* [Acrylic Label Properties] */
// Add a solid shelf under the acrylic on one side for placing a label
Use_Acrylic_Label = true;
// Which side the label shelf is on
Acrylic_Label_Side = 0; // [0: Front (-Y), 90: Right (+X), 180: Back (+Y), 270: Left (-X)]
// How far the shelf extends inward from the edge of the acrylic area (mm)
Acrylic_Label_Depth_mm = 15;
// Distance below z=0 (lid top) for top of shelf. Increase to push down.
Acrylic_Label_Z_Drop_mm = 0.8;
// How thick the shelf is (mm)
Acrylic_Label_Height_mm = 2;
...
// Label shelf: a solid block inside the lid on one side.
// Clipped to BaseShape for rounded corners, then wall geometry
// subtracted so it can never protrude through the sloped walls.
module AcrylicLabelShelf() {
    acrylic_w = Width_mm - 2 * Acrylic_Inset_mm;
    acrylic_l = Length_mm - 2 * Acrylic_Inset_mm;
    depth = Acrylic_Label_Depth_mm;

    is_x_side = (Acrylic_Label_Side == 90 || Acrylic_Label_Side == 270);

    shelf_w = is_x_side ? depth : acrylic_w;
    shelf_l = is_x_side ? acrylic_l : depth;

    pos_x = (Acrylic_Label_Side == 270) ? -acrylic_w / 2 :
            (Acrylic_Label_Side == 90)  ?  acrylic_w / 2 - depth :
            -shelf_w / 2;
    pos_y = (Acrylic_Label_Side == 0)   ? -acrylic_l / 2 :
            (Acrylic_Label_Side == 180) ?  acrylic_l / 2 - depth :
            -shelf_l / 2;

    shelf_top = -Acrylic_Label_Z_Drop_mm;

    difference() {
        intersection() {
            // The shelf block
            translate([pos_x, pos_y, shelf_top - Acrylic_Label_Height_mm])
            cube([shelf_w, shelf_l, Acrylic_Label_Height_mm]);

            // Clip to lid rounded outline
            translate([0, 0, shelf_top - Acrylic_Label_Height_mm])
            linear_extrude(height = Acrylic_Label_Height_mm)
            BaseShape();
        }
        // Subtract ALL wall geometry so shelf never pokes through
        BottomStraights();
        BottomCorners();
    }
}
...
module EntirePart() {
    union() {
        ...
        // Label shelf added after all cuts
        if (Use_Acrylic_Label && Use_Acrylic_Insert) AcrylicLabelShelf();
    }
}
